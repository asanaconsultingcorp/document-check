...
BEGIN
    ...
    --INSERT INTO SCHEDULED_TASK_HEADER
    ...
    FOR record_loop1 in c1 DO
        ...
        sql_query := 'INSERT INTO DOC_AI_DB.SCHEDULED_TASKS.SCHEDULED_TASK_DETAIL(ID, SCHEDULED_TASK_HEADER_ID, BATCH_HEADER_ID, CREATED_BY, MODIFIED_BY) 
            VALUES (''' || detail_uuid || ''', ''' || job_uuid || ''', ''' || current_batch_header_id || ''', CURRENT_USER(), CURRENT_USER())';
        EXECUTE IMMEDIATE :sql_query;
    ...
    */
    ...
    sql_query := 'UPDATE DOC_AI_DB.SCHEDULED_TASKS.SCHEDULED_TASK_HEADER SET END_DATE_TIME=CURRENT_TIMESTAMP() WHERE ID = ''' || job_uuid || '''';
    EXECUTE IMMEDIATE :sql_query;
END;
